...
import {
    parsePercentage8,
    parseFanPowerTachoAggregate,
    encodePercentage8,
    type FanPowerTachoData
} from '../utils/parsers';
...
export class Fan {
    ...
    async discover(): Promise<void> {
        try {
            ...
            const aggregateChar = await this.service.getCharacteristic(
                NEVERMORE_CHARACTERISTICS.FAN_POWER_TACHO_AGGREGATE
            );
            if (aggregateChar) {
                this.aggregate = new NotifyCharacteristic(
                    aggregateChar,
                    parseFanPowerTachoAggregate
                );
            ...
            const allChars = await this.service.getCharacteristics(
                toUuidString(STANDARD_CHARACTERISTICS.PERCENTAGE_8)
            );
                    ...
                    try {
                        ...
                        for (const desc of descriptors) {
                            if (desc.uuid === '00002901-0000-1000-8000-00805f9b34fb') {
                                // User Description UUID
                                const value = await desc.readValue();
                                const description = new TextDecoder().decode(value);

                                // Match "Fan % - Override" description
                                if (
                                    description.includes('Override') &&
                                    !description.includes('Kick-start')
                                ) {
                                    this.powerOverride = new WriteableCharacteristic(
                                        allChars[i],
                                        parsePercentage8,
                                        encodePercentage8
                                    );
