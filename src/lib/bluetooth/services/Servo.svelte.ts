...
import { parsePercentage16, parseServoRange, encodePercentage16, encodeServoRange, type ServoRangeData } from '../utils/parsers';
...
export class Servo {
	...
	async discover(): Promise<void> {
		try {
			...
			const positionChar = await this.service.getCharacteristic(NEVERMORE_CHARACTERISTICS.SERVO_POSITION);
			if (positionChar) {
			...
			const rangeChar = await this.service.getCharacteristic(NEVERMORE_CHARACTERISTICS.SERVO_RANGE);
			if (rangeChar) {
				this.range = new WriteableCharacteristic(rangeChar, parseServoRange, encodeServoRange);
