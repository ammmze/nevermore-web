...
export class EnvironmentalSensing {
    ...
    async discover(): Promise<void> {
        try {
            ...
            if (tempChars[0])
                this.temperatureIntake = new ReadOnlyCharacteristic(tempChars[0], parseTemperature);
            if (tempChars[1])
                this.temperatureExhaust = new ReadOnlyCharacteristic(
                    tempChars[1],
                    parseTemperature
                );
            if (tempChars[2])
                this.temperatureMcu = new ReadOnlyCharacteristic(tempChars[2], parseTemperature);

            if (humidityChars[0])
                this.humidityIntake = new ReadOnlyCharacteristic(humidityChars[0], parseHumidity);
            if (humidityChars[1])
                this.humidityExhaust = new ReadOnlyCharacteristic(humidityChars[1], parseHumidity);

            if (pressureChars[0])
                this.pressureIntake = new ReadOnlyCharacteristic(pressureChars[0], parsePressure);
            if (pressureChars[1])
                this.pressureExhaust = new ReadOnlyCharacteristic(pressureChars[1], parsePressure);

            if (vocIndexChars[0])
                this.vocIndexIntake = new ReadOnlyCharacteristic(vocIndexChars[0], parseVocIndex);
            if (vocIndexChars[1])
                this.vocIndexExhaust = new ReadOnlyCharacteristic(vocIndexChars[1], parseVocIndex);

            if (vocRawChars[0])
                this.vocRawIntake = new ReadOnlyCharacteristic(vocRawChars[0], parseVocRaw);
            if (vocRawChars[1])
                this.vocRawExhaust = new ReadOnlyCharacteristic(vocRawChars[1], parseVocRaw);
        } catch (e) {
